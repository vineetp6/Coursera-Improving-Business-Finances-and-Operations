---
title: "[Managerial Accounting: Tools for Facilitating and Guiding Business Decisions](https://www.coursera.org/learn/operations-management/home/welcome) : Module 2 Mini-Project"
...
dat <- data_frame(Category = c('Immediate Outflow', 'Cash Savings', 'Year 1', 'Year 2', 'Year 3', 'Year 4', 'Year 5', 'Year 6'), `Option A` = c(100000, NA, 10000, 50000, 20000, 70000, 80000, 10000), `Option B` = c(250000, NA, 1000, 2000, 3000, 1000, 20000, 390000)) %>% mutate(`Playback A` = c(-`Option A`[1:2], cumsum(`Option A`[-c(1:2)]) - `Option A`[1]), `Playback B` = c(-`Option B`[1:2], cumsum(`Option B`[-c(1:2)]) - `Option B`[1]))
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/8Ldmh1Qethw" frameborder="0" allowfullscreen></iframe>
  ...
  - *Payback A* has turned to be positive return at `r as.character(na.omit(dat[dat$'Playback A' > 0,])[1, 1])`, therefore we using last negative figure Year + `r na.omit(dat[dat$'Playback A' <= 0,])['Playback A'] %>% .[nrow(.), ] %>% abs %>% as.character` ÷ `r na.omit(dat[dat$'Playback A' >= 0, ])['Option A'] %>% .[1, ] %>% abs %>% as.character` = `r paste0(as.character(na.omit(dat[dat$'Playback A' <= 0,])[1] %>% .[nrow(.),]), abs(na.omit(dat[dat$'Playback A' <= 0,])['Playback A'] %>% .[nrow(.),] / na.omit(dat[dat$'Playback A' >= 0, ])['Option A'] %>% .[1, ]) %>% round(2) %>% substr(2, 4))`.
  - *Payback B* has turned to be positive return at `r as.character(na.omit(dat[dat$'Playback B' > 0,])[1, 1])`, therefore we using last negative figure Year + `r na.omit(dat[dat$'Playback B' <= 0,])['Playback B'] %>% .[nrow(.),] %>% abs %>% as.character` ÷ `r na.omit(dat[dat$'Playback B' >= 0,])['Option B'] %>% .[1,] %>% abs %>% as.character` = `r paste0(as.character(na.omit(dat[dat$'Playback B' <= 0,])[1] %>% .[nrow(.),]), abs(na.omit(dat[dat$'Playback B' <= 0,])['Playback B'] %>% .[nrow(.),] / na.omit(dat[dat$'Playback B' >= 0, ])['Option B'] %>% .[1, ]) %>% round(2) %>% substr(2, 4))`.
...
npv <- dat[1:3] %>% mutate(`NPV A` = round(`Option A` * 1 / 1.16^c(0, 0, seq(1, 6)), 2), `NPV B` = round(`Option B` * 1 / 1.16^c(0, 0, seq(1, 6)), 2))

dat1b <- data.frame(Category = 'Total in 6 Years', t(data.frame(colSums(npv[-c(1:2), -1]))))
rownames(dat1b) <- NULL

npv <- rbind(data.frame(npv), dat1b)
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/kDPmQ6dNxTk" frameborder="0" allowfullscreen></iframe>
  ...
  Lets assume that the annual return rate is 16%.

  - *NPV* for Option A = `r as.character(tail(npv$NPV.A, 1))` - `r as.character(npv$NPV.A[1])` = `r as.character(tail(npv$NPV.A, 1) - npv$NPV.A[1])`, therefore it is a profitable investment.
  - *NPV* for Option B = `r as.character(tail(npv$NPV.B, 1))` - `r as.character(npv$NPV.B[1])` = `r as.character(tail(npv$NPV.B, 1) - npv$NPV.B[1])`, therefore it is NOT a profitable investment.
...
## Part 2
  ...
  - <s> *IRR* for project A... project B...</s>
...
## Part 3
  ...
  Describe some of the strengths and weaknesses of your analysis (i.e., specific measures, etc.). Also, what other considerations might influence your recommendation?

  I am not understanind *IRR* even though read some references. I've just based on the cashflow and return rate to cover the invested cost, the project A will be faster to make even and made profit.
  
  With consider of cashflow issue, project A is better than project B.
...
# 3. Conclusion
  ...
  Project A will be better than project B for investment.
